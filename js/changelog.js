...
(function () {
  ...
  var LOG = [
    { date: '2026-09-11', title: '加载性能大幅优化', desc: '全站图片从 28.4 MB 压缩到 7.9 MB（约省 20 MB）：星空与天体大图全部转为 WebP，并按实际显示尺寸重压，观感无损而体积减至约五分之一；「仰望星空」的背景改为按尺度按需加载，进入该页的首次流量从约 4 MB 降至约 0.25 MB，向外缩放时相邻背景自动补齐，不会出现空档；公式渲染库 KaTeX 改为本站自托管，不再依赖国外 CDN，任何网络环境下公式都能稳定显示；「分享当前视图」生成的链接现在可直接打开到对应视图（含虚无·图景的各个子页），普通刷新则仍从时间线开始。' },
